#!/bin/bash
#
# run.sh [provider] [kube_version] [extra-docker-args]

#    ex: to add explicit ip of provider api endpoint
#    run.sh 127 "--add-host identity.outscale.local:10.1.1.1"
#

PACKER_PROVIDER="${1:? provider needed (openstack,outscale)}"
kube_version="${2:-}"
docker_args=" $3 "
IMAGE_BUILDER_VERSION="${IMAGE_BUILDER_VERSION:-v0.1.30}"

packer_provider_json=./packer.$PACKER_PROVIDER.json
packer_provider_rc=./packer.$PACKER_PROVIDER.rc
if [[ -f "$packer_provider_json" && -f "$packer_provider_rc"  ]]; then
  echo "$packer_provider_json , $packer_provider_rc OK"
else
  echo "$packer_provider_json , $packer_provider_rc not found"
  exit 1
fi
